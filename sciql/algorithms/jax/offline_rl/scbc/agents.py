...
from sciql.algorithms.jax.offline_rl.scbc.samplers.samplers import SCBC_Batch, Dummy_Sampler
from sciql.algorithms.jax.offline_rl.scbc.networks import GLCDiscreteActor, GLCActor
from sciql.algorithms.jax.offline_rl.scbc.encoders import gc_encoder_modules, GCEncoder, lc_encoder_modules
...
class SCBC_TrainState(NamedTuple):
    actor: TrainState

class SCBC_Config(BaseModel):
    """
...
class SCBC_Agent(flax.struct.PyTreeNode, Agent):

    rng: jax.random.PRNGKey
    eval_rng: jax.random.PRNGKey
    eval_labels: Any = nonpytree_field()
    config: SCBC_Config = nonpytree_field()
    train_state: SCBC_TrainState
    ...
    def create(
        ...
        config: Optional[SCBC_Config] = None,
    ):  
        # Get config from cfg or use the provided config object
        if config is None:
            if cfg is None: cfg = {}
            config = SCBC_Config(**cfg)
        ...
        # Get data from batch
        example_batch: SCBC_Batch = sampler.sample_batch(1, batch_rng)
    ...
    @classmethod
    def load(cls, path: str) -> "SCBC_Agent":
        """
        ...
        data_structure = SCBC_Batch(**reconstructed_dict)
        ...
        config = SCBC_Config(**metadata['config'])
    ...
    def update_actor(
        self,
        train_state: SCBC_TrainState, # Actor needs access to all critics/values
        batch: SCBC_Batch,
    ...
    @jax.jit
    def update_n_steps(self, sampler: Sampler) -> Tuple["SCBC_Agent", Dict]:
        """
    ...
    def to(self, device: str) -> "SCBC_Agent":
