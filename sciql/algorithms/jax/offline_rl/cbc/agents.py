...
from sciql.algorithms.jax.offline_rl.cbc.samplers.samplers import CBC_Batch, Dummy_Sampler
from sciql.algorithms.jax.offline_rl.cbc.networks import GLCDiscreteActor, GLCActor
from sciql.algorithms.jax.offline_rl.cbc.encoders import gc_encoder_modules, GCEncoder, lc_encoder_modules
...
class CBC_TrainState(NamedTuple):
    actor: TrainState

class CBC_Config(BaseModel):
    """
...
class CBC_Agent(flax.struct.PyTreeNode, Agent):

    rng: jax.random.PRNGKey
    eval_rng: jax.random.PRNGKey
    eval_labels: Any = nonpytree_field()
    config: CBC_Config = nonpytree_field()
    train_state: CBC_TrainState
    ...
    @classmethod
    def load(cls, path: str) -> "CBC_Agent":
        """
        ...
        data_structure = CBC_Batch(**reconstructed_dict)
        ...
        config = CBC_Config(**metadata['config'])

        # 5. Create the abstract agent.
    ...
    def update_actor(
        self,
        train_state: CBC_TrainState, # Actor needs access to all critics/values
        batch: CBC_Batch,
    ) -> Tuple[TrainState, Dict]:

        def actor_loss_fn(actor_params: flax.core.FrozenDict) -> jnp.ndarray:
            dist = train_state.actor.apply_fn(actor_params, batch.observations, labels=batch.labels)
    ...
    @jax.jit
    def update_n_steps(self, sampler: Sampler) -> Tuple["CBC_Agent", Dict]:
        """
    ...
    def to(self, device: str) -> "CBC_Agent":
