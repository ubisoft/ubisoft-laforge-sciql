...
from sciql.algorithms.jax.offline_rl.sciql_joint.gating import sigmoid_beta
from sciql.algorithms.jax.offline_rl.sciql_joint.samplers.samplers import MINE_Batch, SCIQL_JOINT_Batch, SCIQL_JOINT_Data, Dummy_Sampler
from sciql.algorithms.jax.offline_rl.sciql_joint.networks import GLCDiscreteActor, GLCActor, GLCDiscreteCritic, GLCValue
from sciql.algorithms.jax.offline_rl.sciql_joint.encoders import gc_encoder_modules, GCEncoder, lc_encoder_modules
...
class SCIQL_JOINT_TrainState(NamedTuple):
    mine: TrainState
    label_critic: TrainState
    label_target_critic: TrainState
    label_value: TrainState
    task_critic: TrainState
    task_target_critic: TrainState
    task_value: TrainState
    actor: TrainState
    adv_min_ema: jnp.ndarray # Shape [2, 1]
    adv_max_ema: jnp.ndarray # Shape [2, 1]

class SCIQL_JOINT_Config(BaseModel):
    """
    ...
    adv_norm_enable: bool = False          # turn on EMA normalization
    adv_norm_low_q: float = 0.05           # lower quantile (for A_min)
    adv_norm_high_q: float = 0.95          # upper quantile (for A_max)
    adv_norm_alpha: float = 0.005          # EMA update rate
    adv_norm_symmetric: bool = False       # if True, use symmetric ±M via |A| quantile
    adv_norm_use_norm_in_weight: bool = True  # if True, use A_norm in exp weight; else keep raw A with beta
    adv_norm_init_abs: float = 1.0         # initial bound if no history
    adv_norm_eps: float = 1e-6             # numerical epsilon
    adv_norm_type: str = 'scaling'         # Apply bounded normalization or scaling normalization
    
class SCIQL_JOINT_Agent(flax.struct.PyTreeNode, Agent):

    rng: jax.random.PRNGKey
    eval_rng: jax.random.PRNGKey
    eval_labels: Any = nonpytree_field()
    config: SCIQL_JOINT_Config = nonpytree_field()
    train_state: SCIQL_JOINT_TrainState
    obs_mean: Any = nonpytree_field()
    obs_std: Any = nonpytree_field()
    n_criteria: Any = nonpytree_field()
    ...
    def create(
        ...
        config: Optional[SCIQL_JOINT_Config] = None,
    ):  
        # Get config from cfg or use the provided config object
        if config is None:
            if cfg is None: cfg = {}
            config = SCIQL_JOINT_Config(**cfg)

        # Set jax seed
        rng = jax.random.PRNGKey(config.seed)
        rng, eval_rng, batch_rng, mine_rng, label_value_rng, label_critic_rng, task_value_rng, task_critic_rng, actor_rng = jax.random.split(rng, 9)

        # Get data from batch
        example_batch: SCIQL_JOINT_Batch = sampler.sample_batch(1, batch_rng)
        observations = example_batch.observations
        actions = example_batch.actions
        labels = example_batch.actor_labels
        ...
        label_value = TrainState.create(
            apply_fn=value_model.apply,
            params=value_model.init(label_value_rng, observations, labels=labels),
        ...
        label_critic = TrainState.create(
            apply_fn=critic_model.apply,
            params=critic_model.init(label_critic_rng, observations, actions=actions, labels=labels),
            tx=optax.adam(learning_rate=config.critic_lr),
        )
        label_target_critic = TrainState.create(
            apply_fn=critic_model.apply,
            params=critic_model.init(label_critic_rng, observations, actions=actions, labels=labels),
            tx=optax.adam(learning_rate=config.critic_lr),
        )
        
        # Initilize task modules
        ...
        train_state = SCIQL_JOINT_TrainState(
            mine=mine,
            label_critic=label_critic,
            label_target_critic=label_target_critic,
            label_value=label_value,
            task_critic=task_critic,
            task_target_critic=task_target_critic,
            task_value=task_value,
            actor=actor,
            adv_min_ema=jnp.full(shape=(2, 1), fill_value=-config.adv_norm_init_abs, dtype=jnp.float32),
            adv_max_ema=jnp.full(shape=(2, 1), fill_value=config.adv_norm_init_abs, dtype=jnp.float32),
        )
        ...
        return cls(
            ...
            n_criteria=len(sampler.labels_numbers),
    ...
    @classmethod
    def load(cls, path: str) -> "SCIQL_JOINT_Agent":
        """
        ...
        data_structure = SCIQL_JOINT_Batch(**reconstructed_dict)
        ...
        config = SCIQL_JOINT_Config(**metadata['config'])
    ...
    @jax.jit
    def update_mine_n_steps(self, sampler: Sampler) -> tuple["SCIQL_JOINT_Agent", dict]:
        """
    ...
    # Policy Training #
    ###################
    def update_label_value_one_step(
        self,
        ts_value: TrainState,
        ts_target_critic: TrainState,
        batch: SCIQL_JOINT_Batch
    ) -> Tuple[TrainState, Dict]:
        
        q1, q2 = ts_target_critic.apply_fn(ts_target_critic.params, batch.observations, actions=batch.actions, labels=batch.value_labels)
        q = jax.lax.stop_gradient(jnp.minimum(q1, q2))

        def value_loss_fn(value_params: flax.core.FrozenDict) -> jnp.ndarray:
            v = ts_value.apply_fn(value_params, batch.observations, labels=batch.value_labels)
            value_loss = expectile_loss(q - v, self.config.expectile).mean()
            return value_loss

        new_value, v_loss = update_by_loss_grad(ts_value, value_loss_fn)
        return new_value, {'loss': v_loss}

    def update_label_critic_one_step(
        self,
        ts_critic: TrainState,
        ts_value: TrainState,
        batch: SCIQL_JOINT_Batch,
        rewards: jnp.ndarray,
    ) -> Tuple[TrainState, Dict]:
        
        next_v = ts_value.apply_fn(ts_value.params, batch.next_observations, labels=batch.value_labels)
        target_q = rewards + self.config.discount * batch.label_masks * next_v
        
        def critic_loss_fn(critic_params: flax.core.FrozenDict) -> jnp.ndarray:
            q1, q2 = ts_critic.apply_fn(critic_params, batch.observations, actions=batch.actions, labels=batch.value_labels)
            critic_loss = ((q1 - target_q) ** 2 + (q2 - target_q) ** 2).mean()
            return critic_loss

        new_critic, c_loss = update_by_loss_grad(ts_critic, critic_loss_fn)
        return new_critic, {'loss': c_loss}
    
    def update_task_critic(
            self, 
            train_state: SCIQL_JOINT_TrainState, 
            batch: SCIQL_JOINT_Batch
        ) -> Tuple["SCIQL_JOINT_TrainState", Dict]:
    ...
    def update_adv_ema(
        ...
        """Update EMA of advantage bounds using batch quantiles (stop-grad)."""

        # Compute robust batch bounds
        if self.config.adv_norm_symmetric:
            m_batch = jnp.quantile(jnp.abs(adv), self.config.adv_norm_high_q, axis=0) # (N+1,)
            a_min_b = -m_batch
            a_max_b = +m_batch
        else:
            a_min_b = jnp.quantile(adv, self.config.adv_norm_low_q, axis=0) # (N+1,)
            a_max_b = jnp.quantile(adv, self.config.adv_norm_high_q, axis=0) # (N+1,)

        a_min_b = jax.lax.stop_gradient(a_min_b.astype(jnp.float32))[:, None]
        a_max_b = jax.lax.stop_gradient(a_max_b.astype(jnp.float32))[:, None]

        # Ensure sane ordering & minimum span
        span = jnp.maximum(a_max_b - a_min_b, self.config.adv_norm_eps)
        a_min_b = jnp.minimum(a_min_b, a_max_b - self.config.adv_norm_eps)
        a_max_b = a_min_b + span

        # EMA update
        alpha = self.config.adv_norm_alpha
        new_min = (1.0 - alpha) * train_state.adv_min_ema + alpha * a_min_b
        new_max = (1.0 - alpha) * train_state.adv_max_ema + alpha * a_max_b
        
        infos = {
                'label_adv_min_ema': new_min[0],
                'label_adv_max_ema': new_max[0],
                'label_adv_min_batch_q': a_min_b[0],
                'label_adv_max_batch_q': a_max_b[0],
                'task_adv_min_ema': new_min[1],
                'task_adv_max_ema': new_max[1],
                'task_adv_min_batch_q': a_min_b[1],
                'task_adv_max_batch_q': a_max_b[1]
            }

        return new_min, new_max, infos

    def update_actor_one_step(
        self,
        train_state: SCIQL_JOINT_TrainState, # Actor needs access to all critics/values
        batch: SCIQL_JOINT_Batch
    ) -> Tuple[TrainState, Dict]:
        
        # Compute labels advantages
        label_v = train_state.label_value.apply_fn(train_state.label_value.params, batch.observations, labels=batch.actor_labels)
        label_q = jnp.minimum(*train_state.label_target_critic.apply_fn(train_state.label_target_critic.params, batch.observations, actions=batch.actions, labels=batch.actor_labels))
        label_advs = jnp.stack([label_q - label_v], axis=-1)
        infos = {
            'label_adv_mean': jnp.mean(label_advs, axis=0),
            'label_adv_min': jnp.min(label_advs, axis=0),
            'label_adv_max': jnp.max(label_advs, axis=0)
        }
        ...
        # Update EMA
        if self.config.adv_norm_enable:
            all_advs = jnp.concatenate([label_advs, task_advs], axis=-1) # (B, N+1)
            new_ema_min, new_ema_max, ema_infos = self.update_adv_ema(train_state, all_advs)
            infos.update(ema_infos)

            # Normalize advs
            if self.config.adv_norm_type == 'bound':
                # bounded normalization in [-1, 1]
                # but can change the sign and the relative ratios between advantages
                # better with asymetric quantiles
                denom = jnp.maximum(new_ema_max - new_ema_min, self.config.adv_norm_eps)  # (N,1)
                all_advs_norm = 2.0 * jnp.clip((all_advs[..., None] - new_ema_min) / denom, 0.0, 1.0) - 1.0  # (B,N,1)
            else:
                # scaling normalization by division to stay within a good range
                # doesn't change the sign or the relative ratios between advantages
                # better with symetric quantiles
                denom = jnp.maximum(new_ema_max, self.config.adv_norm_eps)  # (N,1)
                all_advs_norm = all_advs[..., None] / denom

            label_advs_norm, task_advs_norm = all_advs_norm[:, :-1, 0], all_advs_norm[:, -1:, 0]
            if self.config.adv_norm_use_norm_in_weight:
                label_advs, task_advs = label_advs_norm, task_advs_norm
            infos.update({
                'label_adv_norm_mean': jnp.mean(label_advs_norm, axis=0),
                'label_adv_norm_min': jnp.min(label_advs_norm, axis=0),
                'label_adv_norm_max': jnp.max(label_advs_norm, axis=0),
                'task_adv_norm_mean': jnp.mean(task_advs_norm),
                'task_adv_norm_min': jnp.min(task_advs_norm),
                'task_adv_norm_max': jnp.max(task_advs_norm)
            })
        else:
            new_ema_min, new_ema_max = train_state.adv_min_ema, train_state.adv_max_ema
            infos.update({
                'label_adv_min_ema': 0.0,
                'label_adv_max_ema': 0.0,
                'label_adv_min_batch_q': 0.0,
                'label_adv_max_batch_q': 0.0,
                'task_adv_min_ema': 0.0,
                'task_adv_max_ema': 0.0,
                'task_adv_min_batch_q': 0.0,
                'task_adv_max_batch_q': 0.0,
                'label_adv_norm_mean': 0.0,
                'label_adv_norm_min': 0.0,
                'label_adv_norm_max': 0.0,
                'task_adv_norm_mean': 0.0,
                'task_adv_norm_min': 0.0,
                'task_adv_norm_max': 0.0
            })

        # Computing the gated advantage
        if self.config.tree == 'reward > joint_label':
            gated_adv = task_advs + sigmoid_beta(task_advs) * label_advs
        elif self.config.tree == 'joint_label > reward':
            gated_adv = label_advs + sigmoid_beta(label_advs) * task_advs
        elif self.config.tree == 'joint_label':
            gated_adv = label_advs
        elif self.config.tree == 'reward':
            gated_adv = task_advs
        else:
            raise ValueError('Unsupported tree')
        
        # Ensure the shape of the advantage and compute weight
        gated_adv = gated_adv.reshape(-1)
        beta_adv = gated_adv * self.config.beta
        exp_a = jnp.exp(beta_adv)
        clipped_exp_a = jnp.minimum(exp_a, 100.0)
        weights = clipped_exp_a

        def actor_loss_fn(actor_params: flax.core.FrozenDict) -> jnp.ndarray:
            dist = train_state.actor.apply_fn(actor_params, batch.observations, labels=batch.actor_labels)
        ...
        infos.update({
            'loss': a_loss,
            'gated_adv_mean': jnp.mean(gated_adv),
            'gated_adv_min': jnp.min(gated_adv),
            'gated_adv_max': jnp.max(gated_adv),
        ...
        return new_actor, new_ema_min, new_ema_max, infos
    ...
    def update_policy_one_step_jitted(
        ...
        batch: SCIQL_JOINT_Batch,
        rng: jax.random.PRNGKey
    ) -> Tuple["SCIQL_JOINT_TrainState", Dict, jax.random.PRNGKey]:
        """
        ...
        # Compute rewards and styles for this specific label
        if self.config.label_prob_type != 'ind':

            if self.config.label_prob_type == 'mine':
                t_value = train_state.mine.apply_fn(train_state.mine.params, batch.observations, actions=batch.actions, labels=batch.value_labels)
                p_value = self.labels_probs[batch.value_labels].squeeze(-1) * jnp.exp(t_value)

            elif self.config.label_prob_type == 'prob_sigmoid':
                p_value = train_state.mine.apply_fn(train_state.mine.params, batch.observations, actions=batch.actions, labels=batch.value_labels) + 1e-8

            elif self.config.label_prob_type == 'prob_softmax':
                p_value = jnp.take_along_axis(train_state.mine.apply_fn(train_state.mine.params, batch.observations, actions=batch.actions, labels=batch.value_labels), batch.value_labels, axis=1).squeeze(axis=1) + 1e-8
            
            rewards = p_value
        else:
            rewards = jnp.all(batch.value_labels == batch.curr_labels, axis=-1)
        rewards = rewards - 1

        # Update the i-th value function
        new_value, value_infos = self.update_label_value_one_step(train_state.label_value, train_state.label_target_critic, batch)
        all_losses.update({f'value_label_{k}': v for k, v in value_infos.items()})
        
        # Update the i-th critic function using the NEWLY updated value function
        new_critic, critic_infos = self.update_label_critic_one_step(train_state.label_critic, new_value, batch, rewards)
        all_losses.update({f'critic_label_{k}': v for k, v in critic_infos.items()})
        
        # Update all style target critic networks
        new_target_critic = self.target_update(new_critic, train_state.label_target_critic, self.config.tau)

        # Update
        train_state = train_state._replace(label_value=new_value, label_critic=new_critic, label_target_critic=new_target_critic)
        ...
        new_actor, new_ema_min, new_ema_max, actor_infos = self.update_actor_one_step(train_state, batch)
        all_losses.update({f'actor_{k}': v for k, v in actor_infos.items()})
        
        # Assemble the final training state
        train_state = train_state._replace(actor=new_actor, adv_min_ema=new_ema_min, adv_max_ema=new_ema_max)
        
        return train_state, all_losses

    @jax.jit
    def update_policy_n_steps(self, sampler: Sampler) -> Tuple["SCIQL_JOINT_Agent", Dict]:
        """
        ...
        loss_accumulator_template = {
            # Value
            'value_label_loss': 0.0,
            'value_task_loss': 0.0,
            # Critic
            'critic_label_loss': 0.0,
            'critic_task_loss': 0.0,
            # Actor raw advantages
            'actor_label_adv_mean': 0.0,
            'actor_label_adv_min': 0.0,
            'actor_label_adv_max': 0.0,
            'actor_task_adv_mean': 0.0,
            'actor_task_adv_min': 0.0,
            'actor_task_adv_max': 0.0,
            # Actor normalized advantages
            'actor_label_adv_norm_mean': 0.0,
            'actor_label_adv_norm_min': 0.0,
            'actor_label_adv_norm_max': 0.0,
            'actor_task_adv_norm_mean': 0.0,
            'actor_task_adv_norm_min': 0.0,
            'actor_task_adv_norm_max': 0.0,
            # Actor advantages EMA
            'actor_label_adv_min_ema': 0.0,
            'actor_label_adv_max_ema': 0.0,
            'actor_label_adv_min_batch_q': 0.0,
            'actor_label_adv_max_batch_q': 0.0,
            'actor_task_adv_min_ema': 0.0,
            'actor_task_adv_max_ema': 0.0,
            'actor_task_adv_min_batch_q': 0.0,
            'actor_task_adv_max_batch_q': 0.0,
            # Actor gated advantages
            'actor_gated_adv_mean': 0.0,
            'actor_gated_adv_min': 0.0,
            'actor_gated_adv_max': 0.0,
            'actor_loss': 0.0,
    ...
    def to(self, device: str) -> "SCIQL_JOINT_Agent":
